...
interface BulkSessionInspectorProps {
  ruleId: string;
...
export default function BulkSessionInspector({ ruleId, analysisRunId, onClose }: BulkSessionInspectorProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Bulk Session Inspector"
    >
      {/* Backdrop */}
      ...
      <div className="relative w-full max-w-lg overflow-y-auto border-l border-slate-700/50 bg-slate-900 p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-100">Bulk Fix Inspector</h2>
          <button
            onClick={onClose}
          ...
          <>
            ...
            <div className="mb-4 rounded-lg border border-slate-700/50 bg-slate-800/50 p-3 text-xs">
              ...
              <div className="mt-1 flex justify-between">
                ...
                <span className="text-slate-300 font-mono">{session.ruleId}</span>
              </div>
              <div className="mt-1 flex justify-between">
                <span className="text-slate-400">Total Findings</span>
                <span className="text-slate-300">{session.totalFindings}</span>
              </div>
              <div className="mt-1 flex justify-between">
                <span className="text-slate-400">Fixed</span>
                <span className="text-emerald-400">{session.fixedCount}</span>
              </div>
              {session.failedCount > 0 && (
                <div className="mt-1 flex justify-between">
                  <span className="text-slate-400">Failed</span>
                  <span className="text-red-400">{session.failedCount}</span>
                </div>
              )}
              <div className="mt-1 flex justify-between">
